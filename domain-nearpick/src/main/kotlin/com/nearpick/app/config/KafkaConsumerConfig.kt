...
open class KafkaConsumerConfig(
    ...
    @Bean
    open fun policyReviewConsumerFactory(): ConsumerFactory<String, PolicyReviewRequestedEvent> {
        val deserializer = JsonDeserializer(PolicyReviewRequestedEvent::class.java)
        deserializer.addTrustedPackages("com.nearpick.app.domain.policy.event")
        deserializer.setUseTypeMapperForKey(false)
    ...
    @Bean
    open fun policyReviewListenerContainerFactory(
        policyReviewConsumerFactory: ConsumerFactory<String, PolicyReviewRequestedEvent>,
        kafkaTemplate: KafkaTemplate<String, PurchaseCreatedEvent>
    ): ConcurrentKafkaListenerContainerFactory<String, PolicyReviewRequestedEvent> {
        val recoverer = DeadLetterPublishingRecoverer(kafkaTemplate)
        val errorHandler = DefaultErrorHandler(recoverer, FixedBackOff(1000L, 3L))

        errorHandler.setRetryListeners(object : RetryListener {
            override fun failedDelivery(record: org.apache.kafka.clients.consumer.ConsumerRecord<*, *>, ex: Exception, deliveryAttempt: Int) {
                log.warn("[PolicyReviewRetry] 재시도 {}/3, topic={}, offset={}, error={}",
                    deliveryAttempt, record.topic(), record.offset(), ex.message)
            }
        })

        val factory = ConcurrentKafkaListenerContainerFactory<String, PolicyReviewRequestedEvent>()
        factory.consumerFactory = policyReviewConsumerFactory
        factory.containerProperties.ackMode = ContainerProperties.AckMode.MANUAL
        factory.setCommonErrorHandler(errorHandler)
        return factory
    }
